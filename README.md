...
# Токени боти худро ворид кунед
BOT_TOKEN = "7903594220:AAFkMN6c8mqM0tVJwYzb2Q4R_yv_C0z5ZnM"
...
async def place_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Мы приняли ваш заказ! В ближайшее время с вами свяжется наш менеджер.")

    # Хабар ба Telegram (чати канала ё ID чат)
    order_info = f"Новый заказ от {update.message.from_user.username}:\n{user_data}"
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^↔️ Обмен и Возврат$"), exchange_return_info))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📲 ВК Сообщество$"), vk_community))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📲 Telegram$"), telegram_info))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📞 Связь со мной$"), contact_info))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^📝 Отзывы$"), feedback))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT & filters.Regex("^⬅️ Назад$"), back))  # Илова кардани функсия
    app.add_handler(MessageHandler(filters.TEXT, place_order))  # Функсияи нав барои фармоиш
